...
echo -e "\n--> Bootstrapping Pythonkindergarten\n"
...
echo -e "\n--> Deploying the Pythonkindergarten stack to the cluster\n"
ssh \
    -o 'StrictHostKeyChecking no' \
    root@$(terraform output -raw pythonkindergarten-swarm-leader-ip-address) \
    -i ssh_key/terraform \
    'docker stack deploy minitwit -c pythonkindergarten_stack.yml'

echo -e "\n--> Done bootstrapping Pythonkindergarten"
...
echo -e "--> You can check the status of swarm cluster @ http://$(terraform output -raw pythonkindergarten-swarm-leader-ip-address):8888"
echo -e "--> ssh to swarm leader with 'ssh root@\$(terraform output -raw pythonkindergarten-swarm-leader-ip-address) -i ssh_key/terraform'"
